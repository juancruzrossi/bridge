---
name: bridge-quick
description: "[Bridge SDD] Fast-path for small tasks — mini-interview, inline plan, execute, and light review in one pass. No files generated. Part of the bridge plugin — invoke via /bridge:quick"
---

# Bridge Quick — Fast-Path Execution

Self-contained fast-path for small, clear tasks. No files are generated (no BRIEF.md, PLAN.md, or .bridge/ artifacts). The entire cycle happens in the conversation.
...
Do NOT use for: multi-module features, architectural changes, or tasks requiring stakeholder alignment. Use the full bridge flow for those.
...
Rules:
- Keep it to 1-6 items. If more than 6, this task is too big for quick — suggest the full bridge flow instead.
...
- This is the complete cycle. Do NOT hand off to other bridge commands.
- Do NOT generate any files in `.bridge/` or elsewhere as process artifacts.
- If during execution you realize the task is bigger than expected, stop and suggest switching to the full bridge flow.
